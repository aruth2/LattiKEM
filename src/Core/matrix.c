...
void matrixmultiplication(double *output, double *leftmatrix, double *rightmatrix, int m, int n, int o)
{
	...
	int im,in,io;
	double *values = (double *)malloc(m*o*sizeof(double));
...
double *transpose(double *matrix, int m, int n)
{
	...
	double *transposedmatrix = (double *)malloc(m * n * sizeof(double));
	int im,in;
	for(im = 0;im<m;im++)
	for(in = 0;in<n;in++)
...
double singlecofactor(double *matrix, int size, int a, int b)
{
	...
	//printmatrix(matrix,size,size);
			...
			double *submatrix = (double *)malloc((size-1)*(size-1)*sizeof(double));
			double value;
...
double *cofactor (double *matrix, int size)
{
	//Constructs a cofactor matrix from a matrix
		...
		double *outputmatrix = (double *)malloc(size*size*sizeof(double));
