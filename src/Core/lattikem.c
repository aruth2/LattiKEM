...
void performTrajectory(Trajectory *traj)
{
	...
	if(traj->step == 0)//Only save the crystal as initial if it is on the first step.
		crys_makexyz(crys,strcat2(traj->dir,"initial.xyz"));	
	threadwait();
...
void simulateTrajectories()
{
	...
	Trajectory *trajectories = malloc(numRuns*sizeof(Trajectory));
	pthread_t *threads = malloc(numRuns*sizeof(pthread_t));
...
void stepAverage(Trajectory *trajectories)
{
	 ...
	 * */
	int numSteps = getNumSteps();
	Trajectory *combinedTrajectory=malloc(sizeof(Trajectory));
	...
	Configuration *singleStepConfigs = malloc(averagingSteps*sizeof(Configuration));
	Configuration *stepAveragedConfigs = malloc(numRuns*sizeof(Configuration));
	Configuration *runAveragedConfig = malloc(sizeof(Configuration));
	...
	for(iMod=0;iMod<modulationSteps;iMod++)
	{
		for(iTraj=0;iTraj<numRuns;iTraj++)
		{
			...
			threadwait();//This will be poorly-parallelized when averagingSteps<numthreads
...
void timeAverage(Trajectory *trajectories)
{
	 ...
	 * */
	 int numSteps = getNumSteps();
	Trajectory *combinedTrajectory=calloc(1,sizeof(Trajectory));
	int modulationSteps = numSteps/averagingSteps;
	...
	Configuration *singleStepConfigs = calloc(2*numRuns*averagingSteps,sizeof(Configuration));
	Configuration *timeAveragedConfigs = calloc(averagingSteps,sizeof(Configuration));
	Configuration *runAveragedConfig = calloc(1,sizeof(Configuration));
	...
	for(iMod=0;iMod<modulationSteps;iMod++)
	{
		...
		threadwait();//This will be much better parallelized for when numRuns<numThreads, however it will use a signficantly higher amount of memory for a large number of runs. Scaling is numRuns*averagingSteps*configSize.
		for(iStepMod=0;iStepMod<averagingSteps;iStepMod++)
		{
