...
char *strcat2(char *str1, char *str2)
{
	...
	int l1 = strlen(str1);
	int l2 = strlen(str2);
	char *output = (char *)malloc((l1+l2+1)*sizeof(char));//1 extra char for null terminator
	strcpy(output,str1);
	strcat(output,str2);
	return output;
}
...
int strip(FILE *infile, char *search, char *output,char *delimeter, int fill, int dorewind)
{
	 ...
	 * */
	 ...
	 while ( 1 == fscanf( infile, " %999[^\n]%*[\n]", line ) )
	 {
		 ...
		 {
			 if(!strstr(strstr(line,search),delimeter))//If the delimeter cannot be found after the search, then this is not the correct entry. Look for the entry in another line. 
			 {
				 continue;
			 }
             //printf("found line \n%s\n",line);
...
int insertionSort(int *list, int numItems, int newItem)
{
	int insertionSpot;
	if((insertionSpot = binarysearch(list,newItem,numItems)) >= 0)
    {
        listinsert(list,numItems,insertionSpot,newItem);
    }
    else return -1; //List already contains the item cannot be added again.
...
void readInt(FILE *infile, char *descriptor, int *value, char *commentFlags)
{
    ...
    printf("%s not set using default value of %d\n",descriptor,*value);
    else
    {
...
void readString(FILE *infile, char *descriptor, char *value, char *commentFlags)
{
    ...
    if(strip(infile,descriptor,buffer,"= ",1,1))
    {
    printf("%s not set, using default value of %s\n",descriptor,value);
	}
    else
    {
	clipComments(buffer,commentFlags);
...
void readDouble(FILE *infile, char *descriptor, double *value, char *commentFlags)
{
    char buffer[1000];
    if(strip(infile,descriptor,buffer,"= ",1,1))
    printf("%s not set, using default value of %g\n",descriptor,*value);
    else
    {
	clipComments(buffer,commentFlags);
    *value = atof(buffer);
...
char *formatStep(int step, int maxSteps)
{
	...
	char *stepString = (char *)calloc((numformat+1),sizeof(char));
...
void listFiles(char *dir, char **filenames, int *numfiles)
{
	...
	*numfiles = 0;
	...
	{
	filenames[*numfiles] = (char *)calloc(strlen(line),sizeof(char));
