...
void (*allocator)(Configuration *);
void (*freer)(Configuration *);
...
void (*configSwapFunction)(Configuration *, int, int);
void (*configPartialSwapFunction)(Configuration *, int, int, int);
...
void LD_listMoves(crystal *crys, LatticeDynamics *LD, MarkovChainMonteCarlo *mcmc)
{
	...
	for(moveType = 0;moveType<LD->numHopPairs;moveType++)
	{
		...
		for(iatom1=species1start;iatom1<species1end;iatom1++)
		{
			...
			{
				iatom2=*(crys->network->adjacencyList+iatom1*crys->network->maxconnections+ineighbor);
...
{
	mcmc->moves= (int *)malloc(2*maxMoves*sizeof(int));
	mcmc->moveBarriers= (double *)malloc(maxMoves*sizeof(double));
    mcmc->moveEnthalpies = (double *)malloc(maxMoves*sizeof(double));
    mcmc->moveRates = (double *)malloc(maxMoves*sizeof(double));
    mcmc->moveProbabilityRanges = (double *)malloc(maxMoves*sizeof(double));
...
//Update the config stored on the thread until it reaches the described step
void traj_configAtStep(Configuration *config, int *usedMoves, int initialStep, int targetStep, int hasNetwork)
{
	if(configSwapFunction == NULL)
		return;
	
	int iStep;
	//Move forward until reaching the correct step
	//printf("Moving config from step %d to step %d\n",initialStep,targetStep);
    for(iStep=initialStep;iStep<targetStep;iStep++)
    {
		configSwapFunction(config,*(usedMoves+2 * iStep),*(usedMoves+2 * iStep+1));
	}
    
    //Move backwards until reaching the correct step        
    for(iStep=initialStep;iStep>targetStep;iStep--)
    {
		configSwapFunction(config,*(usedMoves+2 * (iStep-1)),*(usedMoves+2 * (iStep-1)+1));
	}    
    //We may need to do this even if configSwapFunction is NULL.
    config->step=targetStep;        
}
...
void traj_registerSettings()
{
	...
	registerInt(&fileAppend,"fileAppend",0);	
...
void LD_setup(double (newEnergyFunction)(Configuration *), void (newSavingFunction)(Configuration *), void (newAllocator)(Configuration *),
 void (newFreer)(Configuration *), void (newAveragingFunction)(Configuration *,int, Configuration *), void (newWeightedAveragingFunction)(Configuration *,int, Configuration *, double *), 
 void (newtraj_generator)(Trajectory *), int newDataSize, void (newConfigSwapFunction)(Configuration *, int, int),void (newConfigPartialSwapFunction)(Configuration *, int, int, int))
{
	...
	configSwapFunction = newConfigSwapFunction;
	configPartialSwapFunction = newConfigPartialSwapFunction;
...
void config_allocate(Configuration *config, Trajectory *traj)
{
	...
	config->seriesData=(double *)malloc(traj->numSeriesData*sizeof(double));
	allocator(config);
	config->step = 0;
	config->savedata = 0;
}
...
//Wrapper function for performing a swap when the config contains additional information that needs to be swapped. e.g. partial sums
void config_performSwap(Configuration *config, int atom1, int atom2, int forward_reverse)
{
	configPartialSwapFunction(config,atom1,atom2,forward_reverse);
}
...
int traj_load(Trajectory *traj)
{
	...
	int maxconnections = traj->crys->network->maxconnections;//Save whatever value was set for maxconnections before loading
	traj->crys = crys_readxyz(strcat2(traj->dir,"initial.xyz"));
	traj->crys->network->maxconnections = maxconnections;
...
void traj_saveSeriesData(Trajectory *traj)
{
	...
	{
		editMode = "w";
		filename = strcat2(traj->dir,*(traj->seriesDataNames+iData));
		
		if(fileAppend)
		{
		editMode = "a";
		numLines = countFileLines(filename);
		}	
	
		//printf("Saving series data file %s\n",*(traj->seriesDataNames+iData));
		seriesDataFile = fopen(filename,editMode);
		for(iStep=numLines;iStep<traj->step;iStep++)
		{
		if(hopMode == HOP_MODE_METROPOLIS)
...
void traj_saveExternalConditions(Trajectory *traj)
{
	int iStep,iCondition;
	//printf("Saving %d series data files\n",traj->numSeriesData);
	
	char *editMode = "w";
	
	int numLines = 0;
	char *filename = strcat2(traj->dir,"External_Conditions");
	...
	FILE *conditionsDataFile = fopen(filename,editMode);
	for(iStep=numLines;iStep<numSteps;iStep++)
	{
		fprintf(conditionsDataFile,"%d",iStep);
		for(iCondition=0;iCondition<traj->numExternalConditions;iCondition++)	
		{
			fprintf(conditionsDataFile," %.15g",*(traj->externalConditions+iStep*traj->numExternalConditions+iCondition));
		}
		fprintf(conditionsDataFile,"\n");
	}
	fclose(conditionsDataFile);
}
...
void traj_loadMoves(Trajectory *traj)
{
	...
	traj->step=fmin(traj->step,fmin(numLines,numSteps));//Take the minimum from the previous loadEnergy and current loadMoves.
...
void traj_saveMoves(Trajectory *traj)
{
	...
	int numLines = 0;
	char *filename = strcat2(traj->dir,"moves");
	...
	FILE *movesfile = fopen(filename,editMode);
	for(i=numLines;i<traj->step;i++)
...
void traj_saveEnergy(Trajectory *traj)
{
	...
	int numLines = 0;
	char *filename = strcat2(traj->dir,"energy");
	...
	FILE *energyfile = fopen(filename,editMode);
...
{
	traj->dir = (char *)calloc(1000,sizeof(char));
 ...
 void traj_initialize(Trajectory *traj)
{
	...
	maxMoves *= traj->crys->network->maxconnections;
	MarkovChainMonteCarlo *mcmc = traj->mcmc = (MarkovChainMonteCarlo *)malloc(sizeof(MarkovChainMonteCarlo));
	mcmc_allocate(mcmc,maxMoves);
	
	//cn_printAdjacencyList(traj->crys);
	
	traj->energies = (double *)calloc(numSteps,sizeof(double));
	traj->timeseries = (double *)calloc(numSteps,sizeof(double));
	traj->timeSteps = (double *)calloc(numSteps,sizeof(double));
	traj->selectedMoves = (int *)calloc(2*numSteps,sizeof(int));
	traj->numrejectedhops=0;
    
    *(traj->timeseries) = 0;
    *(traj->timeSteps) = 0;
    traj->step = 0;
    traj->breakpoints = (int *)malloc(numSteps*sizeof(int));//lil overkill but it works
	traj->numbreakpoints = 0;
