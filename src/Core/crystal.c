...
{
	crys->species = calloc(crys->totalAtoms * namelength, sizeof(char));
	crys->positions = calloc(3*crys->totalAtoms,sizeof(double));
	crys->totalEachSpecies = calloc(maxelements,sizeof(int));
	crys->elements= calloc(maxelements*namelength,sizeof(char));
...
crystal * crys_multiply(struct crystal *base, int m1, int m2, int m3)
{
	...
	crystal *output = malloc(sizeof(crystal));
...
crystal *crys_duplicate(crystal *crys)
{
	...
	crystal *outcrys = malloc(sizeof(crystal));
...
crystal * crys_combine(struct crystal *crys1, struct crystal *crys2, double *offset, int addvectors)
{
	...
	crystal *newcrystal = malloc(sizeof(crystal));
...
crystal * crys_linearInterpolation(crystal *crys1, crystal *crys2, int numimages)
{
	...
	if(numimages == 0)
	return NULL;
	crystal *newcrystals = malloc(numimages*sizeof(crystal));
...
crystal *crys_readxyz(char *name)
{
	...
	crystal *crys = malloc(sizeof(crystal));
...
void crys_makeVASP(struct crystal *crys, char *name, int cart)
{
 ...
 "%s";
	...
	else
	{
		...
		double *fractionalcoordinates = malloc(crys->totalAtoms*3*sizeof(double));
...
{
	struct crystal *crys = malloc(sizeof(struct crystal));
...
crystal * crys_maxDefectSpacing(crystal *inputcrys, char **atoms,int numremove)
{
	...
	iterations[i] = 0;
	combinations[i] = malloc(numtoremove[i]*sizeof(int));
...
crystalnetwork *crys_duplicateNetwork(crystal *crys)
{
	...
	crystalnetwork *newcn = malloc(sizeof(crystalnetwork));
	//newcn->numVacancies = crys->network->numVacancies;
	newcn->adjacencyList = malloc(maxconnections*crys->totalAtoms*sizeof(int));
	newcn->numAdjacent = malloc(crys->totalAtoms*sizeof(int));
	...
	*(newcn->adjacencyList+i*maxconnections+j) = *(crys->network->adjacencyList+i*maxconnections+j);
	}
...
void crys_replaceRandomAtoms(crystal *crys, int numReplace, char *replaceElements, int numReplaceElements, char *newElement)
{
	...
	{
		iatom = (crys->totalAtoms)*generaterandom();//The atom to change to vacancy
		while(crys_elementInString(replaceElements,numReplaceElements,crys->species+iatom*namelength) == -1)
		iatom = (crys->totalAtoms)*generaterandom();
		//printf("Changing atom %d which is element %s to a vacancy\n",j,crys->species+j*namelength);
		
		x=*(crys->positions+3*iatom);
		y=*(crys->positions+3*iatom+1);
		z=*(crys->positions+3*iatom+2);
